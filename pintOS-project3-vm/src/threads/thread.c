...
//  sema_init (&(initial_thread->die_sema), 0);
...
thread_exit (void) 
{
  ...
  if(!t->is_waiting) {
   // printf("엄마가 기다리기 전에 얼릉 죽어야징 ㅎㅎ\n");
    sema_down(&t->zombie_sema);
  }

 // printf(" thiread_exit 할거니까 exitcode 리턴해! 나는 %d\n",thread_current()->tid);
  sema_up(&(t->exit_sema));
 
  sema_down(&(t->die_sema));

//  printf(" 바로 thread_ die!! 한다!!! 내이름은 %d\n", thread_current()->tid);

 /*  while(!list_empty(&t->children)){
     elem = list_pop_front(&(t->children));     
